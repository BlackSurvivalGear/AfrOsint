setTimeout(()=>{let s=document.getElementById('splashScreen');s.classList.add('fade-out');setTimeout(()=>s.remove(),500)},5000);
...
w.innerHTML='<span style="color:#44ff8888;font-size:9px;font-family:Share Tech Mono,monospace">🔑</span><input id="countryPanelApiKey" type="password" placeholder="OpenAI API key (sk-...)" style="width:160px;padding:4px 6px;background:#081821;border:1px solid #44ff8833;color:#d7ffff;font-size:10px;border-radius:3px;font-family:Share Tech Mono,monospace;box-sizing:border-box" /><button onclick="_cpApiKeySet()" style="padding:4px 6px;background:#081821;border:1px solid #44ff8844;color:#44ff88;font-size:10px;cursor:pointer;border-radius:3px;font-family:Share Tech Mono,monospace;margin-left:2px">SET</button>';
}
}
...
<div id='aiCardDragHandle' style='cursor:move;padding:6px 12px;border-bottom:1px solid #00ffee22;display:flex;align-items:center;justify-content:space-between;user-select:none'><span style='color:#00ffee;font-family:Share Tech Mono,monospace;font-size:10px;letter-spacing:2px;text-shadow:0 0 6px #00ffee66'>⚡ AI REPORTS</span><span id='aiCardCollapseArrow' style='color:#00ffee88;font-size:9px;cursor:pointer;padding:2px 4px'>▼</span></div>
<div id='aiCardBody' style='padding:8px 12px'>
<div id='cpApiKeyWrap' style='display:flex;align-items:center;gap:4px;margin-bottom:8px'></div>
<div style='display:flex;flex-wrap:wrap;gap:6px'><button class='atw-small-button' onclick="aiCountrySitrepMaster('${escapedName}');document.getElementById('africaTimePanel').classList.remove('open');africaTimePanelOpen=false" style='background:#0c2430;border-color:#ff4444;color:#ff4444;text-shadow:0 0 6px #ff444488;font-size:10px;padding:5px 10px'>📋 SitRep</button><button class='atw-small-button' onclick="aiStrategicRelocation('${escapedName}');document.getElementById('africaTimePanel').classList.remove('open');africaTimePanelOpen=false" style='background:#0c2430;border-color:#cc44ff;color:#cc44ff;text-shadow:0 0 6px #cc44ff88;font-size:10px;padding:5px 10px'>🏠 RELOCATE</button><button class='atw-small-button' onclick="alert('Coming soon')" style='background:#0c2430;border-color:#00ffee33;color:#00ffee55;font-size:10px;padding:5px 10px;opacity:0.5'>📊 ECON</button><button class='atw-small-button' onclick="alert('Coming soon')" style='background:#0c2430;border-color:#00ffee33;color:#00ffee55;font-size:10px;padding:5px 10px;opacity:0.5'>🛡️ SECURITY</button><button class='atw-small-button' onclick="alert('Coming soon')" style='background:#0c2430;border-color:#00ffee33;color:#00ffee55;font-size:10px;padding:5px 10px;opacity:0.5'>🏗️ INFRA</button></div>
</div>
</div>
...
_renderCpApiKey();
(function(){var card=document.getElementById('aiCardPanel');var handle=document.getElementById('aiCardDragHandle');if(!card||!handle)return;var dragging=false,didDrag=false,offX=0,offY=0,startMX=0,startMY=0;function toggleCollapse(){var b=document.getElementById('aiCardBody');var a=document.getElementById('aiCardCollapseArrow');if(!b||!a)return;if(b.style.display==='none'){b.style.display='block';a.textContent='\u25bc'}else{b.style.display='none';a.textContent='\u25b6'}}handle.addEventListener('mousedown',function(e){e.preventDefault();dragging=true;didDrag=false;startMX=e.clientX;startMY=e.clientY;var r=card.getBoundingClientRect();var pr=card.offsetParent.getBoundingClientRect();offX=e.clientX-(r.left-pr.left);offY=e.clientY-(r.top-pr.top)});document.addEventListener('mousemove',function(e){if(!dragging)return;if(Math.abs(e.clientX-startMX)>3||Math.abs(e.clientY-startMY)>3)didDrag=true;var pr=card.offsetParent;var prR=pr.getBoundingClientRect();var x=e.clientX-prR.left-offX;var y=e.clientY-prR.top-offY;x=Math.max(0,Math.min(x,pr.scrollWidth-card.offsetWidth));y=Math.max(0,Math.min(y,pr.scrollHeight-card.offsetHeight));card.style.left=x+'px';card.style.top=y+'px';card.style.right='auto'});document.addEventListener('mouseup',function(){if(dragging&&!didDrag)toggleCollapse();dragging=false});handle.addEventListener('touchstart',function(e){var t=e.touches[0];startMX=t.clientX;startMY=t.clientY;didDrag=false;var r=card.getBoundingClientRect();var pr=card.offsetParent.getBoundingClientRect();offX=t.clientX-(r.left-pr.left);offY=t.clientY-(r.top-pr.top);dragging=true},{passive:true});document.addEventListener('touchmove',function(e){if(!dragging)return;var t=e.touches[0];if(Math.abs(t.clientX-startMX)>3||Math.abs(t.clientY-startMY)>3)didDrag=true;var pr=card.offsetParent;var prR=pr.getBoundingClientRect();var x=t.clientX-prR.left-offX;var y=t.clientY-prR.top-offY;x=Math.max(0,Math.min(x,pr.scrollWidth-card.offsetWidth));y=Math.max(0,Math.min(y,pr.scrollHeight-card.offsetHeight));card.style.left=x+'px';card.style.top=y+'px';card.style.right='auto'},{passive:true});document.addEventListener('touchend',function(){if(dragging&&!didDrag)toggleCollapse();dragging=false})})();
...
h+="<div style='display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:#081821;border:1px solid #00ffee33;border-radius:6px'>";
h+="<span style='color:#7fd6df;font-family:Share Tech Mono,monospace;font-size:13px'>1 "+t+"</span>";
h+="<span style='color:#ffcc00;font-family:Share Tech Mono,monospace;font-size:15px;font-weight:bold'>"+crossRate.toFixed(2)+" "+currTicker+"</span></div>"});
h+="</div>";
h+="<div id='tvChartWrap' style='margin-top:16px;height:400px;border:1px solid #00ffee33;border-radius:8px;overflow:hidden'><div class='tradingview-widget-container' style='height:100%;width:100%'><div class='tradingview-widget-container__widget' style='height:calc(100% - 32px);width:100%'></div></div></div>";
panel.innerHTML=h;
...
var embedSrc=ch.embed.indexOf('youtube.com')>=0?(ch.embed+'?autoplay=1&mute=0'):ch.embed;
...
function showPopupFxRates(){_autoShowFx=true;popupSetCountry(_lastPopupInfo.name);afrMapInstance.closePopup()}
function showPopupBugout(){_autoShowBugout=true;popupSetCountry(_lastPopupInfo.name);afrMapInstance.closePopup()}
...
if(afrMapInstance)afrMapInstance.closePopup();
...
var iso3=c.cca3||'';var latlng=L.latLng(lat,lng);
if(afrMapInstance){afrMapInstance.flyTo(latlng,5,{duration:1.2})}
...
var afrMapInstance=null;var afrConflictLayer=null;var afrConflictVisible=false;var afrBasesLayer=null;var afrBasesVisible=false;var afrShippingLayer=null;var afrShippingVisible=false;var afrOilLayer=null;var afrOilVisible=false;var afrDisputedLayer=null;var afrDisputedVisible=false;var afrLangLayer=null;var afrLangVisible=false;var afrTileLayer=null;var afrUseGoogle=false;var afrRegionIdx=0;var afrRegions=[{name:'AFRICA',lat:5,lng:20,z:4},{name:'NORTH',lat:32,lng:10,z:5},{name:'WEST',lat:10,lng:-2,z:5},{name:'SAHEL',lat:15,lng:18,z:5},{name:'CENTRAL',lat:-5,lng:28,z:5},{name:'EAST',lat:2,lng:38,z:5},{name:'SOUTHERN',lat:-25,lng:28,z:5},{name:'HORN',lat:12,lng:44,z:5},{name:'CARIBBEAN',lat:18,lng:-70,z:5},{name:'MIDDLE EAST',lat:30,lng:44,z:5}];
const afrLangZones=[
// English-speaking
...
renderCountryPopup(info,latlng);
}).catch(function(){renderCountryPopup(info,latlng)});
}else{renderCountryPopup(info,latlng)}
}).catch(function(){
renderCountryPopup({name:name,official:'',capital:'N/A',population:'N/A',region:'N/A',subregion:'',area:'N/A',languages:'N/A',currencies:'N/A',currCodes:[],callingCode:'N/A',flag:'',timezone:'N/A',continent:'N/A',exchangeRate:null,aes:['Mali','Burkina Faso','Niger'].indexOf(name)>=0},latlng);
});
}
function renderCountryPopup(info,latlng){
if(!afrMapInstance)return;
_lastPopupInfo=info;
var flagHtml=info.flag?'<img src="'+info.flag+'" onclick="popupSetCountry(\''+info.name.replace(/'/g,"\\'")+'\');afrMapInstance.closePopup()" style="width:36px;height:auto;border-radius:3px;border:1px solid #00ffee44;vertical-align:middle;margin-right:8px;cursor:pointer" title="Open country panel">':'';
...
function toggleAfrLayer(id){const m={conflict:{layer:'afrConflictLayer',vis:'afrConflictVisible'},bases:{layer:'afrBasesLayer',vis:'afrBasesVisible'},shipping:{layer:'afrShippingLayer',vis:'afrShippingVisible'},oil:{layer:'afrOilLayer',vis:'afrOilVisible'},disputed:{layer:'afrDisputedLayer',vis:'afrDisputedVisible'},lang:{layer:'afrLangLayer',vis:'afrLangVisible'}};const cfg=m[id];if(!cfg)return;const lyr=window[cfg.layer];if(window[cfg.vis]){afrMapInstance.removeLayer(lyr);window[cfg.vis]=false}else{lyr.addTo(afrMapInstance);window[cfg.vis]=true}if(id==='bases'){const leg=document.getElementById('baseLegend');if(leg)leg.style.display=window[cfg.vis]?'block':'none'}if(id==='lang'){const leg=document.getElementById('langLegend');if(leg)leg.style.display=window[cfg.vis]?'block':'none'}}
var panelDragEl=null,panelDragOX=0,panelDragOY=0;
function startPanelDrag(e,panelId){var tag=e.target.tagName.toLowerCase();if(tag==='span'&&e.target.style.cursor==='pointer')return;e.preventDefault();var touch=e.touches?e.touches[0]:e;panelDragEl=document.getElementById(panelId);var rect=panelDragEl.getBoundingClientRect();var section=panelDragEl.parentElement.getBoundingClientRect();panelDragOX=touch.clientX-rect.left;panelDragOY=touch.clientY-rect.top;panelDragEl.style.right='auto';panelDragEl.style.left=(rect.left-section.left)+'px';panelDragEl.style.top=(rect.top-section.top)+'px';document.addEventListener('mousemove',doPanelDrag);document.addEventListener('mouseup',stopPanelDrag);document.addEventListener('touchmove',doPanelDrag,{passive:false});document.addEventListener('touchend',stopPanelDrag)}
function doPanelDrag(e){if(!panelDragEl)return;e.preventDefault();var touch=e.touches?e.touches[0]:e;var section=panelDragEl.parentElement.getBoundingClientRect();var x=touch.clientX-panelDragOX-section.left;var y=touch.clientY-panelDragOY-section.top;panelDragEl.style.left=Math.max(0,x)+'px';panelDragEl.style.top=Math.max(0,y)+'px'}
function stopPanelDrag(){document.removeEventListener('mousemove',doPanelDrag);document.removeEventListener('mouseup',stopPanelDrag);document.removeEventListener('touchmove',doPanelDrag);document.removeEventListener('touchend',stopPanelDrag);panelDragEl=null}
...
var afrSafeZoneLayer=null,afrRiskyZoneLayer=null,afrSafeVisible=false,afrRiskyVisible=false;
var safeZones=[
...
if(!afrMapInstance)return;
afrSafeZoneLayer=L.layerGroup();
safeZones.forEach(function(z){L.circle([z.lat,z.lng],{radius:z.radius,color:'#00ff66',fillColor:'#00ff66',fillOpacity:0.12,weight:1.5,opacity:0.6}).bindPopup('<div style="font-family:Share Tech Mono,monospace;font-size:12px"><strong style="color:#00ff66">'+z.name+'</strong><br>'+z.info+'</div>').addTo(afrSafeZoneLayer)});
afrRiskyZoneLayer=L.layerGroup();
riskyZones.forEach(function(z){L.circle([z.lat,z.lng],{radius:z.radius,color:'#ff4444',fillColor:'#ff4444',fillOpacity:0.10,weight:1.5,opacity:0.5,dashArray:'6,4'}).bindPopup('<div style="font-family:Share Tech Mono,monospace;font-size:12px"><strong style="color:#ff4444">'+z.name+'</strong><br>'+z.info+'</div>').addTo(afrRiskyZoneLayer)});
}
function toggleSurvivalLayer(type){
if(!afrMapInstance)return;
if(!afrSafeZoneLayer)initSurvivalLayers();
if(type==='safe'){
if(afrSafeVisible){afrMapInstance.removeLayer(afrSafeZoneLayer);afrSafeVisible=false}else{afrSafeZoneLayer.addTo(afrMapInstance);afrSafeVisible=true}
}else if(type==='risky'){
if(afrRiskyVisible){afrMapInstance.removeLayer(afrRiskyZoneLayer);afrRiskyVisible=false}else{afrRiskyZoneLayer.addTo(afrMapInstance);afrRiskyVisible=true}
}
...
if(window.L&&window.L.DomEvent){L.DomEvent.disableClickPropagation(panel);L.DomEvent.disableScrollPropagation(panel)}
}}
