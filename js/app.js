...
var vp=document.getElementById('opsViewport');if(vp){vp.style.overflow='hidden';vp.style.touchAction='none'}
...
function stopPanelDrag(e){document.removeEventListener('mousemove',doPanelDrag);document.removeEventListener('mouseup',stopPanelDrag);document.removeEventListener('touchmove',doPanelDrag);document.removeEventListener('touchend',stopPanelDrag);if(!panelDragActive&&panelDragEl&&e&&e.type==='touchend'){e.preventDefault();var handle=panelDragEl.querySelector('[id$="DragHandle"]');if(handle)handle.click()}panelDragEl=null;panelDragActive=false}
