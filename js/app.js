...
_renderCpApiKey();
(function(){var card=document.getElementById('aiCardPanel');var handle=document.getElementById('aiCardDragHandle');if(!card||!handle)return;var dragging=false,offX=0,offY=0;handle.addEventListener('mousedown',function(e){e.preventDefault();dragging=true;var r=card.getBoundingClientRect();var pr=card.parentElement.getBoundingClientRect();offX=e.clientX-(r.left-pr.left);offY=e.clientY-(r.top-pr.top)});document.addEventListener('mousemove',function(e){if(!dragging)return;var pr=card.parentElement.getBoundingClientRect();var x=e.clientX-pr.left-offX;var y=e.clientY-pr.top-offY;x=Math.max(0,Math.min(x,pr.width-card.offsetWidth));y=Math.max(0,Math.min(y,pr.height-card.offsetHeight));card.style.left=x+'px';card.style.top=y+'px';card.style.right='auto'});document.addEventListener('mouseup',function(){dragging=false});handle.addEventListener('touchstart',function(e){var t=e.touches[0];var r=card.getBoundingClientRect();var pr=card.parentElement.getBoundingClientRect();offX=t.clientX-(r.left-pr.left);offY=t.clientY-(r.top-pr.top);dragging=true},{passive:true});document.addEventListener('touchmove',function(e){if(!dragging)return;var t=e.touches[0];var pr=card.parentElement.getBoundingClientRect();var x=t.clientX-pr.left-offX;var y=t.clientY-pr.top-offY;x=Math.max(0,Math.min(x,pr.width-card.offsetWidth));y=Math.max(0,Math.min(y,pr.height-card.offsetHeight));card.style.left=x+'px';card.style.top=y+'px';card.style.right='auto'},{passive:true});document.addEventListener('touchend',function(){dragging=false})})();
